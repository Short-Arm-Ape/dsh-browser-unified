...
export function isPrivateIPv4(addr: string): boolean {
  ...
  if (a === 198 && (b === 18 || b === 19)) return true // benchmark / proxy fake-ip
...
/**
 * Clash / Surge / mihomo "fake-ip" pools live in RFC 2544's 198.18.0.0/15.
 * Real LAN SSRF still uses 10/8, 127/8, 169.254/16, 192.168/16, etc.
 */
...
export interface UrlGuardOptions {
  ...
  /**
   * Allow DNS answers in `198.18.0.0/15` (Clash/Surge fake-ip). Defaults to `true`
   * so desktop users behind a system proxy keep working; real private ranges stay blocked.
   */
  allowFakeIp?: boolean
...
export function createUrlGuard(options: UrlGuardOptions = {}): UrlGuard {
  ...
  const allowFakeIp = options.allowFakeIp ?? true
  ...
  function blockedAsPrivate(addr: string, family: number): boolean {
    if (!isPrivateAddress(addr, family)) return false
    if (allowFakeIp && isFakeIpAddress(addr, family)) return false
    return true
  }
  ...
  return {
    async assertPublicHttpUrl(raw: string): Promise<URL> {
      ...
      if (literalFamily !== 0) {
        if (!allowPrivate && blockedAsPrivate(host, literalFamily)) {
      ...
      for (const entry of resolved) {
        if (blockedAsPrivate(entry.address, entry.family)) {
