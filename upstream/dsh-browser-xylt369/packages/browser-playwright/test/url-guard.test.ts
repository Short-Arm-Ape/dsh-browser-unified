...
import { createUrlGuard, isFakeIpIPv4, isPrivateIPv4, isPrivateIPv6 } from '../src/url-guard.ts'
...
test('isFakeIpIPv4 recognizes Clash/Surge 198.18.0.0/15 pools', () => {
  assert.equal(isFakeIpIPv4('198.18.0.56'), true)
  assert.equal(isFakeIpIPv4('198.19.255.255'), true)
  assert.equal(isFakeIpIPv4('198.17.0.1'), false)
  assert.equal(isFakeIpIPv4('10.0.0.1'), false)
})
...
test('guard allows fake-ip literals by default (Clash DNS)', async () => {
  const guard = createUrlGuard()
  const url = await guard.assertPublicHttpUrl('http://198.18.0.56/')
  assert.equal(url.toString(), 'http://198.18.0.56/')
})

test('guard rejects fake-ip when allowFakeIp is false', async () => {
  const guard = createUrlGuard({ allowFakeIp: false })
  await assert.rejects(() => guard.assertPublicHttpUrl('http://198.18.0.56/'), { code: 'WEB_PRIVATE_TARGET' })
})
