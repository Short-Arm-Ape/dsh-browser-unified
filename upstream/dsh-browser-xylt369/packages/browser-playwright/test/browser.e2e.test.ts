...
test('provider fills, presses, waits, and can go forward after back', async () => {
  const ctx = new Context()
  const fiber = await ctx.plugin(PlaywrightBrowserRuntime)
  const browser = ctx.browser
  ...
  try {
    ...
    const filled = await page.fill('#name-field', 'hello-dsh')
    assert.equal(filled.ok, true)
    const value = await page.evaluate<string>('document.querySelector("#name-field").value')
    assert.equal(value, 'hello-dsh')

    const pressed = await page.press('Tab', '#name-field')
    assert.equal(pressed.ok, true)

    const waited = await page.wait({ ms: 50 })
    assert.equal(waited.ok, true)

    await page.navigate('https://example.com/')
    await page.back()
    const fwd = await page.forward()
    assert.ok(fwd.url.includes('example.com'))
  } finally {
    await fiber.dispose()
  }
})

test('provider isolates tabs per session key', async () => {
  const ctx = new Context()
  const fiber = await ctx.plugin(PlaywrightBrowserRuntime)
  const browser = ctx.browser
  ...
  try {
    const a = await browser.newPage({ windowVisibility: 'headless', profileDir, sessionKey: 's-a' })
    const b = await browser.newPage({ windowVisibility: 'headless', profileDir, sessionKey: 's-b' })
    await a.navigate('https://example.com/')
    await b.navigate('https://example.com/')
    await a.evaluate(`document.title = 'session-a'`)
    await b.evaluate(`document.title = 'session-b'`)
    assert.equal(await a.title(), 'session-a')
    assert.equal(await b.title(), 'session-b')

    const second = await browser.openTab({ sessionKey: 's-a' })
    const tabs = await browser.listTabs('s-a')
    assert.equal(tabs.length, 2)
    assert.equal(tabs.filter((tab) => tab.active).length, 1)
    await browser.switchTab(a.id, 's-a')
    const afterSwitch = await browser.listTabs('s-a')
    assert.equal(afterSwitch.find((tab) => tab.id === a.id)?.active, true)
    await browser.closeTab(second.id, 's-a')
    assert.equal((await browser.listTabs('s-a')).length, 1)
    assert.equal((await browser.listTabs('s-b')).length, 1)
  } finally {
    await fiber.dispose()
  }
})
