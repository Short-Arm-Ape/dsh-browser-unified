 ...
 * @module browser-unified-core/approval
 */
...
export type GateAction =
  | { kind: 'pass' } // not ours / already armed / full-access session / read-only under navigation scope
  | { kind: 'ask-arm' } // the arm tool under approvalMode 'arm': always ask, then arm
  | { kind: 'ask' } // every other gated call
  | { kind: 'disarm' } // the disarm tool: no approval needed
...
export const INTRANET_READ_ONLY_TOOLS: ReadonlySet<string> = new Set([
  'intranet_snapshot',
  'intranet_screenshot',
  'intranet_scroll',
  'intranet_wait',
...
export function gateAction(
  ...
  shape: GateShape = { prefix: 'intranet_' },
): GateAction {
  const armTool = shape.armTool ?? `${shape.prefix}arm`
  const disarmTool = shape.disarmTool ?? `${shape.prefix}disarm`
  if (!toolName.startsWith(shape.prefix)) return { kind: 'pass' } // ① not ours
  if (toolName === disarmTool) return { kind: 'disarm' } // ② disarm needs no approval
  if (policy === 'never') return { kind: 'pass' } // ③ full-access session auto-passes
  if (toolName === armTool) {
  ...
  if (config.approvalScope === 'navigation' && readOnly.has(toolName)) return { kind: 'pass' }
 ...
 */
export function latestSessionPolicy(agent: unknown, fallback: ApprovalPolicy = 'ask'): ApprovalPolicy {
...
export function approvalReason(toolName: string, args: unknown, shape: GateShape = { prefix: 'intranet_' }): string {
