/**
 * Vendored from @caob23/dsh-browser-control v1.0.7 (AGPL-3.0), file src/ws.ts.
 * Copied verbatim; part of the AGPL-3.0-licensed browser-unified-core merge.
 * Upstream: https://github.com/caob23/dsh-browser-control
 *
 ...
 * chunk boundaries.
 * @module browser-unified-core/bridge/ws
 */
...
export class FrameReader {
  ...
  private readFrame(): WsFrame | null {
    ...
    if (length === 126) {
      ...
      offset += 8
      if (extended > BigInt(MAX_FRAME_BYTES)) throw new Error(`websocket frame too large: ${extended} bytes`)
    ...
    if (maskKey) {
      for (let i = 0; i < payload.length; i++) payload[i] = payload[i]! ^ maskKey[i & 3]!
    }
