 ...
 * browser-unified-plugin, AGPL-3.0 (see NOTICE.md).
 */
...
const ENTRY_STATUSES = ['proposed', 'accepted', 'applied', 'rejected', 'superseded'] as const
const TEXT_OUT = {
...
function registryDirOf(configured: string): string {
...
async function readJson(file: string): Promise<Record<string, unknown>> {
	...
	if (typeof parsed !== 'object' || parsed === null) throw new Error(`not a JSON object: ${file}`)
	return parsed as Record<string, unknown>
}

interface GitRun { ok: boolean; out: string; err: string }
function runGit(args: readonly string[]): Promise<GitRun> {
	return new Promise((resolve) => {
		execFile('git', args as string[], { timeout: 25_000, maxBuffer: 1 << 20, windowsHide: true }, (error, stdout, stderr) => {
			resolve({ ok: !error, out: stdout, err: String(stderr || (error && error.message) || '') })
		})
	})
}

function parseHeadSha(out: string): string | undefined {
...
async function gitHeadSha(url: string): Promise<string> {
	...
	if (second.ok && sha2) return sha2
	throw new Error((second.err || first.err).trim().split('\n')[0]?.slice(0, 200) || 'git ls-remote failed')
}

function renderText(_args: unknown, value: { text: string }): Array<{ type: string; text: string }> {
	return [{ type: 'text', text: String(value.text) }]
}

function textResult(text: string): { ok: boolean; text: string } {
	return { ok: true, text }
}

function safe(fn: () => Promise<{ ok: boolean; text: string }>, label: string): Promise<{ ok: boolean; text: string }> {
	return fn().catch((e: unknown) => ({ ok: false, text: `${label}：${e instanceof Error ? e.message : String(e)}` }))
}
...
export function applyUnifiedTools(ctx: Context, config: { registryDir: string }): void {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args: any, exec: any) {
			return safe(async () => {
				...
				const repos = Array.isArray(base.repos) ? base.repos as Array<Record<string, unknown>> : []
				if (repos.length === 0) throw new Error('upstream-baseline.json: repos 为空')
				const wanted = Array.isArray(args.repos) && args.repos.length > 0 ? new Set<string>(args.repos as string[]) : null
				const lines: string[] = []
				let drift = 0
				...
				if (args.includeDesign === true) {
					const entries = Array.isArray(reg.designEntries) ? reg.designEntries as Array<Record<string, unknown>> : []
	...
	ctx.tools.register(defineTool({
		...
		async execute(args: any) {
			return safe(async () => {
				...
				const tooling = (reg.tooling ?? {}) as Record<string, unknown>
				const authz = (reg.authz ?? {}) as Record<string, unknown>
				...
				const rules = (reg.impactRules ?? {}) as Record<string, Array<Record<string, unknown>>>
				const keys = Object.keys(rules)
				lines.push(`影响映射覆盖上游(${keys.length}): ${keys.join(', ')}`)
				if (args.full === true) {
					for (const k of keys) {
						for (const row of rules[k] ?? []) {
							if (row.m) lines.push(`  [${k}] ${String(row.m)} -> ${String(row.t)}`)
						}
					}
				}
				const entries = Array.isArray(reg.designEntries) ? reg.designEntries as Array<Record<string, unknown>> : []
	...
	ctx.tools.register(defineTool({
		...
		async execute(args: any, exec: any) {
			return safe(async () => {
				const reg = await readJson(registryFile) as Record<string, unknown>
				...
				const entries = Array.isArray(reg.designEntries)
					? reg.designEntries as Array<Record<string, unknown>>
					: []
				let preview: string

				if (action === 'add') {
					if (!title || title.length > 200) return { ok: false, text: 'browser_design_edit: add 需要 1..200 字的 title（用户明示）' }
					let max = 0
					for (const e of entries) {
					...
					const entry: Record<string, unknown> = { id, title, status: status || 'proposed', decidedBy: sourceType, createdAt: now, updatedAt: now }
					if (summary) entry.summary = summary.slice(0, 2000)
					...
					if (!args.entryId) return { ok: false, text: 'browser_design_edit: transition 需要 entryId' }
					...
					if (!entry) return { ok: false, text: `browser_design_edit: 找不到条目 ${String(args.entryId)}` }
					const now = new Date().toISOString().slice(0, 10)
					entry.status = status
					entry.updatedAt = now
				...
				const outcome = await approval.request({ agent: { id: agent.id }, toolName: 'browser_design_edit', reason, signal: (exec as { signal?: AbortSignal }).signal })
